...
new_function = '''def generate_debt_dossier_pdf(app, applicant, statements, docs):
    ...
    t_profile = Table(profile_data, colWidths=[100, 155, 100, 160])
    ...
    t_loan = Table(loan_data, colWidths=[110, 145, 100, 160])
...
"""
    ...
    t_agree = Table(agree_data, colWidths=[130, 385])
    ...
    if not statements:
        ...
        t_stmt = Table(stmt_data, colWidths=[65, 55, 170, 70, 70, 85])
...
if match:
    ...
    with open('backend/website/export_service.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print('Patched export_service.py with styled alignment!')
else:
