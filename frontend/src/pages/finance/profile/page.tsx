...
const inputClass =
  "w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-violet-700/20 focus:border-violet-700 transition-colors placeholder:text-gray-300 disabled:bg-slate-50 disabled:text-slate-400";
...
function DocumentRow({
  ...
  return (
    ...
    >
      {/* Icon */}
      <div
        className={`mt-0.5 w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isUploaded ? "bg-violet-100 text-violet-600" : selectedFile ? "bg-blue-100 text-blue-600" : "bg-blue-50 text-blue-600"
          }`}
      >
        {isUploaded ? (
...
export default function FinanceProfilePage() {
  ...
  const companyLabel = role === "FINANCE_IDEON" ? "Ideon Limited" : "Nakama";
  ...
  const refreshProfile = useCallback(async () => {
    if (!userId) return;
    const { data } = await applicantApi.getApplicantDetails(userId);
    if (data?.applicant) {
      setProfile({
        userId,
        name: data.applicant.name ?? "",
        email: data.applicant.email ?? "",
        phoneNumber: data.applicant.phone_number ?? "",
        idNumber: data.applicant.id_number ?? null,
        kraPin: data.applicant.kra_pin ?? null,
        payrollNumber: data.applicant.payroll_number ?? null,
        applicantType: data.applicant.applicant_type ?? "",
        docs: data.docs ?? [],
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      // payroll_number is usually not for finance but just sending empty string if not present
      fd.append("payroll_number", (serverProfile as any)?.payrollNumber || "");
  ...
  const displayName = user?.name ?? "Finance Manager";
  ...
  return (
    <div className="p-6 w-full space-y-6">
      ...
      {/* Avatar + name */}
      <div className="bg-gradient-to-r from-violet-900 to-violet-700 rounded-xl p-6 flex items-center gap-5">
          ...
          {editing ? (
            ...
            >
              ...
              <Button
                type="submit"
                disabled={saving}
                className="bg-violet-700 hover:bg-violet-800 text-white font-bold rounded-xl w-full sm:w-auto"
              >
              ...
              {showUploadButton && (
                <div className="mt-4 flex justify-end">
                  <Button
                    ...
                    className="bg-violet-700 hover:bg-violet-800 text-white font-bold rounded-xl"
                  >
