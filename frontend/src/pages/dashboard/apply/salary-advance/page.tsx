...
import { AlertCircle, CheckCircle2, ChevronLeft, Info, Loader2 } from "lucide-react";
...
const errorClass =
  "text-red-500 text-xs mt-1.5 flex items-center gap-1";
...
export default function SalaryAdvancePage() {
  ...
  const company = searchParams.get("company") ?? "ideon";
  const isIdeon = company === "ideon";
  const formatCompanyLabel = (c: string) => {
    if (c === "ideon") return "Ideon Limited";
    if (c === "nakama") return "Nakama Tech Limited";
    return c.split("_").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
  };
  const companyLabel = formatCompanyLabel(company);
  const loanType: LoanType = isIdeon
  ...
  }, [session?.user?.id, serverProfile?.idNumber, serverProfile?.kraPin, serverProfile?.payrollNumber]);
  ...
  // NaN-safe: ?? doesn't catch NaN, so normalise here
      ...
      <div className="p-6 w-full max-w-2xl mx-auto space-y-6">
        <div className="bg-white rounded-3xl ring-1 ring-slate-200 shadow-sm p-8 flex flex-col items-center text-center gap-4">
          ...
          <p className="text-slate-500 text-sm">
            Your {companyLabel} salary advance application has been received.
        ...
        <Button
          ...
          className="w-full bg-brand-blue hover:bg-slate-800 text-white font-bold h-12 rounded-xl"
        >
          Back to Dashboard
        </Button>
      </div>
    ...
    <div className="p-6 w-full space-y-6 min-h-full">
      ...
      <div className="bg-white rounded-xl ring-1 ring-slate-200 shadow-sm p-6 sm:p-8">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
          {/* ── Personal details (pre-filled, read-only) ── */}
          ...
          <div>
            ...
            {/* Average calculation and max loan info */}
            {salaryCalc && (
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-2 px-4 py-3 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <p className="text-xs text-amber-800">
                  The loan amount is automatically calculated based on your average salary. 
                  The interest rate for salary advances is fixed at {(LOAN_SETTINGS.salaryAdvance.interestRate * 100).toFixed(0)}%.
                </p>
              </div>
            </div>
          )}
