...
interface DocDef {
  key: string;          // FormData field name sent to the backend
  serverType: string;   // document_type returned by the server
  label: string;
  description: string;
}
...
export default function ProfilePage() {
  ...
  const appType = serverProfile?.applicantType ?? user?.company ?? "";
  const showPayroll = appType.toLowerCase() !== "personal" && appType.toLowerCase() !== "";
  ...
  const onSaveProfile = async (data: ProfileForm) => {
    ...
    try {
      ...
      // Update session JWT (non-fatal — proceed even if this fails)
      try {
        await update({
          name: data.name,
          email: data.email,
      ...
      toast.success("Personal info updated successfully!", { duration: 3000 });
      setEditing(false);
      ...
      <div className="bg-white rounded-xl ring-1 ring-slate-200 shadow-sm p-6 flex items-center gap-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className="text-xs bg-blue-100 text-blue-700 font-bold px-2.5 py-1 rounded-full capitalize">
              {user?.role?.toLowerCase()}
            </span>
            {applicantType && (
              <span className="text-xs bg-slate-100 text-slate-600 font-semibold px-2.5 py-1 rounded-full capitalize">
                {applicantType}
              </span>
            )}
          </div>
          ...
          {editing ? (
            ...
            >
              ...
              <Button
                type="submit"
                disabled={saving}
                className="bg-blue-900 hover:bg-slate-800 text-white font-bold rounded-xl w-full sm:w-auto"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {[
                ...
                <div key={label}>
                  ...
                  <p
                    className={`text-sm font-medium ${value === "Not set" ? "text-gray-300" : "text-slate-800"
                      }`}
                  >
                    {value}
                  </p>
              ...
              <div className="flex flex-wrap gap-2">
                {requiredDocs.map((doc) => {
                  ...
                  return (
                    <span
                      key={doc.key}
                      className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${uploaded
