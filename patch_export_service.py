...
new_function = '''def generate_debt_dossier_pdf(app, applicant, statements, docs):
    ...
    title_style = ParagraphStyle('Title', parent=styles['Heading1'], alignment=1, fontSize=16, spaceAfter=20, textColor=colors.HexColor("#1E3A8A"))
    ...
    elements.append(Paragraph(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ParagraphStyle('small', fontSize=8, alignment=1)))
    ...
    profile_data = [
        ["Full Name:", applicant.name, "ID Number:", str(applicant.id_number or 'N/A')],
        ["Email:", applicant.email, "KRA PIN:", str(applicant.kra_pin or 'N/A')],
        ["Phone Number:", applicant.phone_number, "Company:", str(applicant.applicant_type).replace('_', ' ').title()],
        ["Payroll Number:", str(applicant.payroll_number or 'N/A'), "Role:", str(applicant.role).title()]
    ]
    
    t_profile = Table(profile_data, colWidths=[100, 160, 100, 150])
    ...
    loan_data = [
        ...
        ["Employment Nature:", str(app.employment_nature or 'N/A').title(), "Designation:", str(app.designation or 'N/A').title()],
        ["Purpose of Loan:", str(app.loan_purpose or 'N/A').title(), "", ""],
    ...
    if app.loan_status == 'disbursed' and app.notes:
        loan_data.append(["Proof of Disbursement:", str(app.notes), "", ""])
    
    t_loan = Table(loan_data, colWidths=[110, 150, 100, 150])
    t_loan.setStyle(TableStyle([
        ...
        ('BACKGROUND', (2, 0), (2, -1), colors.HexColor("#F1F5F9")),
        ('PADDING', (0, 0), (-1, -1), 6),
        ('SPAN', (1, 6), (3, 6)), # Span Purpose of Loan
    ]))
    # If Proof of disbursement exists, span it
    if app.loan_status == 'disbursed' and app.notes:
        t_loan.setStyle(TableStyle([('SPAN', (1, len(loan_data)-1), (3, len(loan_data)-1))]))
...
"""
    elements.append(Paragraph(tc_text, ParagraphStyle('Agreement', parent=normal_style, fontSize=9, leading=11)))
    elements.append(Spacer(1, 10))
    
    agree_data = [
        ["Electronically Accepted By:", applicant.name],
        ...
        ["Consent Granted:", "Read and accepted Loan Agreement T&Cs. Read and understood Privacy Policy and consented to data processing under the Data Protection Act, 2019."]
    ]
    t_agree = Table(agree_data, colWidths=[130, 380])
    ...
    if not statements:
        ...
        stmt_data = [["Date", "Reference", "Description", "Debit (KES)", "Credit (KES)", "Balance (KES)"]]
        for s in statements:
            stmt_data.append([
        ...
        t_stmt = Table(stmt_data, colWidths=[70, 60, 160, 70, 70, 80])
    ...
    if not docs:
        ...
        for doc_obj in docs:
            elements.append(KeepTogether([
                Paragraph(f"Document Type: {str(doc_obj.document_type).replace('_', ' ').title()}", styles['Heading3']),
                Spacer(1, 10)
            ]))
            ...
            if filepath and os.path.exists(filepath):
                ...
                if ext in ['.png', '.jpg', '.jpeg']:
                    ...
                    elements.append(Paragraph(f"[PDF Document Attached: {os.path.basename(filepath)}]", normal_style))
                    elements.append(Paragraph("Note: PDF attachments are listed here but their pages are not visually merged into this document. The full documents are available in the system.", normal_style))
                else:
                    elements.append(Paragraph(f"[File Attached: {os.path.basename(filepath)}]", normal_style))
            else:
                elements.append(Paragraph("[File missing or inaccessible]", normal_style))
    ...
    if has_pdf_attachments:
        try:
            ...
            # PyPDF2 not installed, return unmerged
            pass
...
if match:
    ...
    with open('backend/website/export_service.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print('Patched export_service.py!')
else:
